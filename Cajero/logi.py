...
def ingresar_contrasena():
    global intentos
    usuario_correcto = input("Ingrese su usuario")
    if usuario_correcto == usuario_correcto:
        return True
    else:
        intentos += 1 
        print(f"usuario incorrecto. Intentos restantes: {intentos_maximos - intentos} ")
        return False
